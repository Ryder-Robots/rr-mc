...
void* EncoderMspStatus::decode(uint8_t *data) {
    ...
    int pos = 0;
    decoded->set_cycletime(ntohs(decodeUint16(data, pos)));
    decoded->set_i2c_errors_count(ntohs(decodeUint16(data, pos)));
    decoded->set_sensor(ntohs(decodeUint16(data, pos)));
