...
uint8_t* EncoderMspStatus::encode(void* data) {
    ...
    int pos = splitUint16(htons(obj->get_cycletime()), encoded, 0);
    pos = splitUint16(htons(obj->get_i2c_errors_count()), encoded, pos);
    pos = splitUint16(htons(obj->get_sensor()), encoded, pos);
    pos = splitUint32(htonl(obj->get_flag()), encoded, pos);
...
void* EncoderMspStatus::decode(uint8_t* data) {
    ...
    int pos = 0;
    decoded->set_cycletime(ntohl(decodeUint16(data, pos)));
    decoded->set_i2c_errors_count(ntohl(decodeUint16(data, pos)));
    decoded->set_sensor(ntohl(decodeUint16(data, pos)));
    decoded->set_flag(static_cast<RRP_STATUS>(ntohl(decodeUint32(data, pos))));
